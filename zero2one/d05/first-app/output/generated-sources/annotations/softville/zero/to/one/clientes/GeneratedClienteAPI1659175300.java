...
@WebResource( path="/api/clientes/inserir/", method="POST" )
final public class GeneratedClienteAPI1659175300 implements HttpHandler {
	...
	private void runMethod( final HttpServerExchange exchange, final byte[] bodyData ){
		if ( exchange.isInIoThread() )
			exchange.dispatch( ()-> this.runMethod( exchange, bodyData ) );
				...
				final kikaha.urouting.api.Response response = instance.inserirCliente( 
			methodDataProvider.getBody( exchange, softville.zero.to.one.clientes.Cliente.class, bodyData ) );
    ...
    public String toString(){
        return "softville.zero.to.one.clientes.ClienteAPI.inserirCliente";
    }
}